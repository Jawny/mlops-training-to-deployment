...
ngc batch run --name "${training_workspace_name}-setup" --preempt RUNONCE --min-timeslice 0s --total-runtime 0s --instance cpu.x86.tiny --commandline "cd /mnt/workspace; git clone ${training_git_url}" --result /results --image "nvidian/pytorch:22.06-py3" --workspace $training_workspace_name:/mnt/workspace:RW
